...
function innerRpc(request, callback) {
  ...
  const errorMessage = 'Xazab JSON-RPC: ';
  ...
  const req = this.protocol.request(options, (res) => {
    ...
    res.on('end', () => {
      ...
      if (res.statusCode === 500 && buf.toString('utf8') === 'Work queue depth exceeded') {
        const exceededError = new Error(`Xazab JSON-RPC: ${buf.toString('utf8')}`);
...
// For definitions of RPC calls, see various files in: https://github.com/xazab/xazab/tree/master/src
RpcClient.callspec = {
